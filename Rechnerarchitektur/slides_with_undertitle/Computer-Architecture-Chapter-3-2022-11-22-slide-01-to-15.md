...
<!-- - `0:17:36`: right you know how this extends so i want to show you already like and you see this am this exile here and this actual curse your all the time because if you recall that the output here it is just third of the sum of of these two input bitrate and here's another x or so this x or. -->
<!-- - `0:17:56`: an actual curse but then you see here on xr and here's another one. -->
<!-- - `0:18:02`: which sums up these two bits plus the carry which comes from this one right and so on so you always get kind of the carry which goes up here in this line and you need to edit with with the result of exploring these two guys rate and and this is already an optimized version where sort of this thing here is a there's a kind of a trick which i don't explain now. -->
<!-- - `0:18:22`: like it uses like one gate less than than sort of the most natural one. -->
...
<!-- - `0:19:58`: so they say if this shift bit is true then take this lifting if it's a forest take this right thing. -->
# ==============0:20:00==============
<!-- - `0:20:06`: okay so i am you see the thing is in hearth very actually while not linear in it like the rebel carry at a resolve before minutes the them and look in em but it has this nice m. -->
<!-- - `0:20:23`: property that the the delay here said of the time it takes you're getting from the input to the output is locked in which is that the number off and which is of course good rates so even for five hundred twelfth you'd like nine layers rate so you would need to go maybe through twenty gates and then. -->
...
- ![new_18](./_Computer-Architecture-Chapter-3-2022-11-22-slide-01-to-15_imgs/new_00:22:51_0010.png)
...
- ![new_19](./_Computer-Architecture-Chapter-3-2022-11-22-slide-01-to-15_imgs/new_00:23:17_0011.png)
...
-------------------------------------------------------------------------------
- ![new_22](./_Computer-Architecture-Chapter-3-2022-11-22-slide-01-to-15_imgs/new_00:24:35_0015.png)
<!-- - `0:24:39`: in great. -->
<!-- - `0:24:41`: this is the carriage she already starts here and goes up here and then goes here like the last output the most significant bit is then dependent on on on on this. -->
<!-- - `0:24:56`: carry chain and then. -->
# ==============0:25:00==============
<!-- - `0:25:00`: sorry. -->
<!-- - `0:25:03`: right in the the the i the reason for that is no matter which of these architecture news these last fro here really needs to have like take all the cory's into account. -->
...
<!-- - `0:25:52`: cycles in and unroll the loop you would also get sixty four times sixty four a there's like no no eggs and there's actually no way of getting it. -->
...
<!-- - `0:26:43`: i'd say you can only compute here this thing after waiting after this toggles now if you think about like or so the two multiplayer right it's like it would need to have like sixty. -->
<!-- - `0:26:56`: gates or soul raid or like sixty likes or gates and. -->
...
<!-- - `0:27:38`: there are multipliers which are actually at the endeavor logarithmic like the bearish if the rate of reloader rhythmic delay and it's pretty surprising that this is possible. -->
...
<!-- - `0:28:27`: and a virus or along so this is the carry safe version rate so so here it's actually kind of linear and and then a but then at the end you have this you need to kind of have this last roll right in the previous leg schematic capture this was the last row and there you have also the the carry chain. -->
<!-- - `0:28:47`: and. -->
...
<!-- - `0:29:02`: how to how can i make this lock in this lower part. -->
...
<!-- - `0:29:15`: and this partial product accumulation is actually pretty simple to to make lock in saw i'll show you this is actually us in this lie original slight set and the tricky part here is to make this sub adder here on top log in. -->
<!-- - `0:29:35`: and then i can already tell you i think i have a picture for that the interesting thing that the trick is an doubt yeah explain this trick down here since it uses kind of a tree idea that simple does this idea this this one top is actually not that triggers i'll show you how this works and the other thing you see here and this is why. -->
- ![new_27](./_Computer-Architecture-Chapter-3-2022-11-22-slide-01-to-15_imgs/new_00:29:38_0020.png)
<!-- - `0:29:54`: i switched the slide is he suddenly this thing is critique right so this is upper triangular matrix for it's quadratic. -->
# ==============0:30:00==============
<!-- - `0:30:00`: but is not bad because you see down you'll be already paid to earn a quadratic number of gates now we make this for a final stage edward this carol the one which had this line. -->
<!-- - `0:30:13`: in the quarterdeck circuit. -->
<!-- - `0:30:15`: again from a linear delay into a logarithmic delay in that's that sir and like an interesting tradeoffs of your trading you see we we have already part of the multiplayer is quadratic. -->
<!-- - `0:30:29`: it's this or adding up this rose led the envy you need to kind of finalize everything in this last carry chain and that was linear in the in the in this low eder they're starting this low multiplier but we can of course paint like portray the space they are cutting many of transistors the pieces. -->
<!-- - `0:30:50`: double the heat of the circuit is we can guarantee then that the the overall instead of distance from the button to the output this is logarithmic. -->
<!-- - `0:31:00`: and i'll explain this in the next light said in this prefix a slight said. -->
<!-- - `0:31:06`: okay so this is the goal so we're having here this is the carry save eta which already uses this trick i showed you here right so this is kind of gives a nicer structure and then this one am but it's still sort of linear right the number of pits up here like the rose. -->
- ![new_28](./_Computer-Architecture-Chapter-3-2022-11-22-slide-01-to-15_imgs/new_00:31:09_0021.png)
<!-- - `0:31:26`: artist linear and and we turn this into a logarithmic computation. -->
<!-- - `0:31:32`: and to and then in addition we turned this long final stage eder which needs to collect all these carries here we go back here in it's collect all these carries which come from the upper row right in and add them together so this line here is then the second part we're going to optimise and put your general. -->
<!-- - `0:31:52`: the logarithmic circuit for it. -->
<!-- - `0:32:00`: alright. -->
...
<!-- - `0:32:43`: read instead of adding always like the previous decide with the next number you just add pierce then you take the you have then edit pearce rate and then you take the result of adding those pills add those together then added like ford numbers and so on break and then you're done in logarithmic number of steps right so here you need like five aiders. -->
...
<!-- - `0:33:32`: you do this to propagate and generate foot for that one i have a slight as as another slight set which is also on the. -->
...
<!-- - `0:35:33`: do you see you you parallelize adding those individual bits by one assembly instructions then for you in the heart that the other paralyzes for you something for which like your software would take lots of steps. -->
...
<!-- - `0:43:27`: now with computing you see that the spec of this thing was i need to compute this last thing this was the spec the sun from one to eight. -->
<!-- - `0:43:35`: i would ignore the rest rate and now i'm turning this algorithm into one arisen which not only computes one to eight but actually computes all the prefix sums okay. -->
<!-- - `0:43:49`: instead of computing one result i'm computing end results. -->
<!-- - `0:43:54`: the some from one to. -->
<!-- - `0:43:56`: i number right. -->
<!-- - `0:43:59`: and you see part of it we did already like is he like it this will be have some one one which is the first element then we added up these two guys and and then the edit up here like the first four and then the fourth aide said like the powers of to we have already another question. -->
...
# ==============0:45:00==============
<!-- - `0:45:04`: and it also only needs em lock in computation time parallel computation time for the spain of this thing this is the main point is logarithmic. -->
<!-- - `0:45:20`: okay so what's the general principle of this while we have an operation which we need to kind of compute on an n objects so we actually we're going to do the ores and anes later for this multiplayer. -->
<!-- - `0:45:34`: and er but there's some addition and we not only want the total sum but we want to have the prefix the subsets of all the sums up to a certain point. -->
...
<!-- - `0:46:24`: okay now if you apply this trick or on a reformulation of the rupert liberal kelly era which are already like i i skipped it was the last slight cold compacted this then you'll get like. -->
<!-- - `0:46:37`: a fast multiplier because this final stage edit would then also only need to have like a span of four logarithmic rate. -->
...
<!-- - `0:47:58`: i hear as i sit like there's some trick going on to reduce the number of gates and in this part here. -->
...
<!-- - `0:48:23`: so this will be sort of than the the simple natural aversion of implementing a ripple carry adders with full interest rate you would get his circuit slightly bigger as i said like one more to pit or so. -->
- ![new_38](./_Computer-Architecture-Chapter-3-2022-11-22-slide-01-to-15_imgs/new_00:48:24_0003.png)
<!-- - `0:48:37`: okay so he are like oars and in here are excerpts this gives the sums and this computes the carries. -->
...
<!-- - `0:50:15`: and. -->
<!-- - `0:50:17`: sorry a carry and this assess if one is true and cover then of course i will propagate the carry if it comes in an industrial. -->
...
<!-- - `0:50:48`: or it was previously generated before and it's propagated because one of the mistral this is a simple recursive definition. -->
...
<!-- - `0:52:03`: propagates like you propagate one to use it even make sense you generate to carry here at the at the last position rate and you propagated all the way to pit ate right and you just write this down for all the all the possible places where you can generate a carry ray. -->
<!-- - `0:52:23`: completely. -->
<!-- - `0:52:25`: intuitive actually. -->
<!-- - `0:52:27`: then this plus years in or so it says like yeah one of these cases has to be true. -->
...
<!-- - `0:53:47`: the things we can do this imperil. -->
...
<!-- - `0:54:43`: okay. -->
<!-- - `0:54:47`: and this aside then would look like this carry a look ahead at her and would tear for quadratic sized editor writes at his final stage edit would suddenly become quartic many gates greg many are transistors but you are happy with this because you're getting like from linear to like. -->
# ==============0:55:00==============
...
<!-- - `0:58:04`: one could even probably claim that the company would go down if they would get into the same situation now of course they now have measures to white this they have proactive measures and they also have some ways of sort of disabling you see they can just load up new microcode which would make the chip slower avoid this complex. -->
